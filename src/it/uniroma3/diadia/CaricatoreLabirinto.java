...
public class CaricatoreLabirinto {
	...
	/* prefisso della riga contenente le stanza magiche */
	private static final String STANZE_MAGICHE_MARKER = "StanzeSpeciali:";
	...
	public void carica() throws FormatoFileNonValidoException {
		try {
			...
			this.leggiEInserisciStanzeMagiche();
	...
	private void leggiEInserisciPersonaggi() throws FormatoFileNonValidoException {
	    ...
	    for (String specificaPersonaggio : separaStringheAlleVirgole(specifichePersonaggi)) {
	        ...
	        try (Scanner scannerLinea = new Scanner(specificaPersonaggio)) {
	            ...
	            check(scannerLinea.hasNext(), msgTerminazionePrecoce("la presentazione del personaggio " + nome + "."));
	            presentazione = scannerLinea.next();

	            check(scannerLinea.hasNext(), msgTerminazionePrecoce("la stanza del personaggio " + nome + "."));
	            nomeStanza = scannerLinea.next();

	            check(this.isStanzaValida(nomeStanza), "Personaggio " + nome + " non collocabile: stanza " + nomeStanza + " inesistente");

	            if (tipo.equals("strega")) {
	                builder.addStrega(nome, presentazione, nomeStanza);
	            }
	                ...
	                try {
	                    ...
	                    builder.addMago(nome, presentazione, nomeStanza, nomeAttrezzo, peso);
	                } catch (NumberFormatException e) {
	                    check(false, "Peso attrezzo del mago " + nome + " non valido");
	                }
	            }
	                ...
	                try {
	                    int peso = Integer.parseInt(pesoAttrezzo);
	                    builder.addCane(nome, presentazione, nomeStanza, nomeAttrezzo, peso, ciboPreferito);
	                } catch (NumberFormatException e) {
	                    check(false, "Peso attrezzo del cane " + nome + " non valido");
	                }
	            }
	...
	private void leggiEInserisciStanzeMagiche() throws FormatoFileNonValidoException {
	    String specificheStanze = this.leggiRigaCheCominciaPer(STANZE_MAGICHE_MARKER);

	    if (specificheStanze.isBlank())
	        return;
	    ...
	    for (String specificaStanza : separaStringheAlleVirgole(specificheStanze)) {
	        ...
	        try (Scanner scannerLinea = new Scanner(specificaStanza)) {
	            ...
	            check(scannerLinea.hasNext(), msgTerminazionePrecoce("il tipo della stanza magica."));
	            tipo = scannerLinea.next();
	            ...
	            if (tipo.equals("buia")) {
	                check(scannerLinea.hasNext(), msgTerminazionePrecoce("l'attrezzo richiesto dalla stanza buia."));
	                String attrezzoRichiesto = scannerLinea.next();

	                builder.addStanzaBuia(nomeStanza, attrezzoRichiesto);
	            }

	            else if (tipo.equals("bloccata")) {
	                check(scannerLinea.hasNext(), msgTerminazionePrecoce("la direzione bloccata."));
	                String direzioneBloccataString = scannerLinea.next();

	                Direzione direzioneBloccata = null;

	                try {
	                    direzioneBloccata = Direzione.valueOf(direzioneBloccataString);
	                } catch (IllegalArgumentException e) {
	                    check(false, "Direzione bloccata non valida: " + direzioneBloccataString);
	                }

	                check(scannerLinea.hasNext(), msgTerminazionePrecoce("l'attrezzo sbloccante."));
	                String attrezzoSbloccante = scannerLinea.next();

	                builder.addStanzaBloccata(nomeStanza, attrezzoSbloccante, direzioneBloccata);
	            }

	            else {
	                check(false, "Tipo stanza magica sconosciuto: " + tipo);
	            }
	        }
	    }
	}
	...
	private void leggiEImpostaUscite() throws FormatoFileNonValidoException {
		...
		try (Scanner scannerDiLinea = new Scanner(specificheUscite)) {			
			...
			while (scannerDiLinea.hasNext()) {
				...
				try {
	                direzione = Direzione.valueOf(dirString);
	            } catch (IllegalArgumentException e) {
	                check(false, "Direzione non valida: " + dirString);
	            }
